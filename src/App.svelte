<script>
	...
	import ONSHeader from "./lib/ONSHeader.svelte";
	import ONSFooter from "./lib/ONSFooter.svelte";
	...
	// $: chartHighlightedReg = metadata.region.array && region ? metadata.region.array.filter(d => d.parent == region).map(d => d.code) : []; // Array of district codes in 'region'
...
<Header bgcolor="#fcaa74" bgfixed={true} theme="dark" center={false} short={true}>
	<h1>Modern web standards with data visualizations</h1>
	<p class="text-big" style="margin-top: 5px">
		Using UK population statistics we will see how visualizations can be made interactive and responsive to all screen sizes
	</p>
	<p style="margin-top: 20px">
		The following uses transition animations, if this upsets you toggle them off
	</p>
	<p>
...
<Filler theme="lightblue" short={true} wide={true} center={false}>
	<p class="text-big">
		This is a demonstration of how we can utilise the power of modern web standards to enhance data storytelling
	</p>
</Filler>
...
<Section>
	...
	<p>
		Often graphs are static with no interactions. These types of visualizations are best suited for print as it doesn't utilise the power of presenting them in modern web browsers.
	</p>
	<p>
		Including these types of visualizations on a website or in a presentation would be to maintain consistency with graphics used in a static report such as a pdf.
	</p>
...
<Section>
	...
	<p>We use a bar chart to quickly rank the regions of the United Kingdom according to population. This gives us a snapshot of the population distribution for mid-2020.</p>
</Section>
...
<div class="chart-sml">
	...
	xKey="pop" yKey="name"
	snapTicks={false}
	xFormatTick={d => (d / 1e6)} xSuffix="m"S
...
<Section>
	<p>
		The chart above is embedded. That is set to the same width as the column, "medium" (680px), but could also be "narrow" (540px), "wide" (980px) or "full" width. All options are responsive to fit the width of narrow screens.
	</p>
...
<Section>
	...
	<p>
		Below is a grid that contain charts or any other kind of visual media. The grid can fit in a medium, wide or full-width column, and the media width itself can be narrow (min 200px), medium (min 300px), wide (min 500px) or full-width. The grid is responsive, and will re-flow on smaller screens.
	</p>

	<p> 
		The above information is expanded to show the popluation trend for the last 20 years. These charts are identical except that each one has a different region highlighted. This allows for an easy comparison of population trends across regions
	</p>
</Section>
...
<Section>
	...
	<p>
		We will repeat the above graphs but do it at a district level. The problem with this idea is that there are too many districts to repeat the graph for each in order to highlight the specific district.
	</p>

	<p> 
To overcome this obstacle we will use our first form of interaction, hovering. We plot all the districts on one graph and when hovering over a particular district it gets highlighted and the district's name is shown.
...
<Media
	col="full"
	height={600}
	caption="This is an optional caption for the above chart or media. It can contain HTML code and hyperlinks, and wrap onto multiple lines."
>
...
<Section>
	<p>
		The visualizations above are static in what they're presenting, with only the last plot adding basic hovering interactions. They are however responsive and will resize depending on the screen size viewed on. In the case of the gridded plots, the layout will change  to adapt to the screen size.
	</p>
</Section>
	...
	<div slot="foreground">
		...
		<section data-id="chart05">
			<div class="col-medium">
				...
				<p>Use the selection box below or click on the chart to select a district. The chart will also highlight the other districts in the same region.</p>
				<p>
					The visualization now becomes interactive as which districts get highlighted is chosen by the user.
				</p>
				{#if geojson}
					<p>
...
<Section>
	...
	<p>
		The map is linked to the scatterplot above. Any district selected in the scatterplot is outlined on the map.
	</p>
</Section>
...
<Section>
	<p>This has been a short example of how we can use modern web protocols to make our data presentations more dynamic</p>
</Section>
